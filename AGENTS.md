...
- Focused dev: `pnpm --filter cv-match-api dev` for the API, `pnpm --filter cv-match-web dev` for the frontend, `pnpm --filter cv-match-reverse-proxy dev` when reloading Caddy.
...
- API unit tests: `pnpm test:api`; coverage: `pnpm --filter cv-match-api test:cov`. Prefer colocated `*.spec.ts` inside feature folders.
